...
const block = `${BEGIN}
Measured ${stamp} by the weekly [probe-vs-hosted workflow](.github/workflows/probe-vs-hosted.yml) against a live deploy — identical requests to this wrapper and to the hosted Linear MCP, response bytes compared per call (per row for lists):
